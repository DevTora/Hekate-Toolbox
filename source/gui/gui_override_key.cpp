...
GuiOverrideKey::GuiOverrideKey() : Gui() {
    ...
    keySelectButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        if (m_inputBlocked)
            gui->drawTextAligned(font20, x + 190, y + 100, currTheme.textColor, "按下任意按键...", ALIGNED_CENTER);
        ...
        gui->drawTextAligned(font14, x + 190, y + 185, currTheme.textColor, "覆盖按键", ALIGNED_CENTER);
    };
    ...
    comboPressedButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        gui->drawTextAligned(font20, x + 20, y + 50, currTheme.textColor, "按键状态：", ALIGNED_LEFT);
        gui->drawTextAligned(font20, x + 360, y + 50, currTheme.selectedColor, m_override.key.overrideByDefault ? "未按下" : "已按下", ALIGNED_RIGHT);
    };
    ...
    switch (g_keyType) {
        case OverrideKeyType::Any_App_Override: {
            ...
            anyTitleIconButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                gui->drawTextAligned(font24, x, y - 60, currTheme.textColor, "进入时覆盖：", ALIGNED_LEFT);
                gui->drawTextAligned(font24, x, y - 20, currTheme.textColor, "任意标题", ALIGNED_LEFT);
            };
            ...
            overrideEnabledButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                gui->drawTextAligned(font20, x + 20, y + 50, currTheme.textColor, "启用", ALIGNED_LEFT);
                gui->drawTextAligned(font20, x + 360, y + 50, m_overrideAnyApp ? currTheme.selectedColor : currTheme.unselectedColor, m_overrideAnyApp ? "开" : "关", ALIGNED_RIGHT);
            };
            ...
            appIconButton->drawAction = [&, title{DumpTitle(m_override.programID)}](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                if (title.get() != nullptr && title->application_id != 0) {
                    ...
                    if (appletName == nullptr) {
                        ...
                        gui->drawTextAligned(font20, Gui::g_framebuffer_width / 2, y + 350, currTheme.activatedColor, "通过 Applet 打开 hbmenu 可能无法为自制软件提供足够内存。\n 建议从游戏中打开 hbmenu 以获得完整 RAM 访问权限。", ALIGNED_CENTER);
                    }
                    ...
                    gui->drawTextAligned(font24, x + 150, y + 280, currTheme.unselectedColor, "未选择标题", ALIGNED_CENTER);
                }
            };
...
void GuiOverrideKey::draw() {
    ...
    Gui::drawTextAligned(font24, 70, 58, currTheme.textColor, "        应用覆盖设置", ALIGNED_LEFT);
