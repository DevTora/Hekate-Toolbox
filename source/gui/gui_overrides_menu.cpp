...
void GuiOverridesMenu::draw() {
    ...
    Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 150, currTheme.textColor, "为每个标题添加按键覆盖选项。\n 按住指定按键组合打开该标题时，将覆盖为 hbmenu。", ALIGNED_CENTER);
...
void GuiOverridesMenu::onInput(u32 kdown) {
    ...
    if (kdown & HidNpadButton_X) {
        ...
        if (buttonType != OverrideButtonType::Add_New) {
            (new MessageBox("确定要删除此设置吗？", MessageBox::YES_NO))
    ...
    switch (buttonType) {
        case OverrideButtonType::Any_Title:
            drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                gui->drawTextAligned(font24, x + 100, y + 285, currTheme.textColor, "任意标题", ALIGNED_CENTER);
            };
            break;
        case OverrideButtonType::Custom_Title:
            drawAction = [&, keyType, title{DumpTitle(key.programID, WidthHeight{192, 192})}](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                gui->drawTextAligned(font24, x + 100, y + 285, currTheme.textColor, keyType == OverrideKeyType::Default ? "默认" : "自定义标题", ALIGNED_CENTER);
            };
            break;
        case OverrideButtonType::Add_New:
            drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                gui->drawTextAligned(fontHuge, x + 100, y + 150, currTheme.unselectedColor, "\uE0F1", ALIGNED_CENTER);
                gui->drawTextAligned(font24, x + 100, y + 285, currTheme.unselectedColor, "添加...", ALIGNED_CENTER);
            };
            ...
            inputAction = [&](u64 kdown, bool *isActivated) {
                if (kdown & HidNpadButton_A) {
                    ...
                    if (displayAnyTitleOption)
                        configNames.push_back("任意标题");

                    if (displayDefaultOption)
                        configNames.push_back("修改默认");

                    if (m_addConfigs.size() != 0)
                        configNames.push_back("自定义标题");

                    (new ListSelector("添加新按键覆盖目标：", "\uE0E1 返回     \uE0E0 确定", configNames, 0))
